...
export default function ProfileSettingsPage() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Profile details successfully saved to PostgreSQL and synchronized across workspaces!');
    } catch (err: any) {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Password successfully modified in secure PostgreSQL database.');
      setCurrentPassword('');
      setNewPassword('');
    } catch (err: any) {
      setError(err.message || 'An error occurred.');
  ...
  return (
    <div className="max-w-4xl mx-auto flex flex-col gap-8">
      ...
      <div>
        ...
        <p className="text-xs text-zinc-400 font-light mt-1">Configure your personal credentials and customize your collaborative avatar.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="glass-panel p-6 rounded-2xl border-white/5 flex flex-col items-center gap-4 relative overflow-hidden h-fit">
          ...
          <div className="w-full flex flex-col gap-2 text-center">
            <span className="text-xs font-bold text-white uppercase">AVATAR GENERATOR</span>
            <input 
              type="text" 
            ...
            />
            <span className="text-[9px] text-zinc-500">Type any seed name to randomize your custom card avatar.</span>
          </div>
        </div>
        ...
        <div className="md:col-span-2 flex flex-col gap-6">
          ...
          <div className="glass-panel p-6 rounded-2xl border-white/5 relative overflow-hidden">
            ...
            <h3 className="text-sm font-bold text-white mb-6 flex items-center gap-1.5">
              <Sparkles className="h-4 w-4 text-secondary" /> Account Specifications
            </h3>
            ...
            <form onSubmit={handleUpdateProfile} className="flex flex-col gap-4">
              ...
              >
                <Save className="h-4 w-4" /> {loading ? 'Saving details...' : 'Save Profile Changes'}
              </button>
            </form>
          </div>
