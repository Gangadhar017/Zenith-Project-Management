...
export default function DashboardPage() {
  ...
  const { currentWorkspace, projects, fetchProjects, toggleStarProject } = useWorkspaceStore();
  ...
  const [aiRisk, setAiRisk] = useState<{
    overallRisk: string;
    riskFactors: Array<{ source: string; severity: string; description: string }>;
  } | null>(null);
  ...
  const filteredProjects = projects.filter((p) => {
    ...
    const matchesStarred = starredFilter === 'ALL' || (starredFilter === 'STARRED' && p.isStarred);
  ...
  const [checklistTasks, setChecklistTasks] = useState([
    { id: '1', title: 'Migrate active database schemas to Neon PostgreSQL', priority: 'HIGH', status: 'OVERDUE', done: false },
    { id: '2', title: 'Bind responsive theme switch toggler to layout', priority: 'HIGH', status: 'ACTIVE', done: true },
    { id: '3', title: 'Verify real-time socket events sync endpoints', priority: 'MEDIUM', status: 'ACTIVE', done: false },
    { id: '4', title: 'Build Clerk account details profile modal overlay', priority: 'HIGH', status: 'ACTIVE', done: false },
    { id: '5', title: 'Polish visual aesthetics and Tailwind slate styles', priority: 'LOW', status: 'ACTIVE', done: false },
  ]);

  const toggleTaskDone = (id: string) => {
    setChecklistTasks(prev => prev.map(t => t.id === id ? { ...t, done: !t.done } : t));
  };
  ...
  useEffect(() => {
    if (currentWorkspace) {
      ...
      fetch(`http://localhost:8000/api/workspaces/${currentWorkspace.id}/stats`, {
      ...
      setLoadingRisk(true);
      fetch(`http://localhost:8000/api/ai/workspace-risks/${currentWorkspace.id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      .then(res => res.json())
    ...
    <div className="flex flex-col gap-8 max-w-7xl mx-auto p-1">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-foreground flex items-center gap-2">
            Welcome back, {user?.name.split(' ')[0] || 'Member'} <span className="animate-waving-hand">👋</span>
          </h1>
          <p className="text-xs text-zinc-500 dark:text-zinc-400 font-light mt-1">Here is the visual operations center for {currentWorkspace?.name || 'your active workspace'} today.</p>
        </div>
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-xs font-semibold text-primary">
          <Sparkles className="h-3.5 w-3.5" /> Professional Enterprise Tier Active
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-panel p-5 rounded-2xl relative overflow-hidden flex items-center justify-between group hover:border-primary/30 transition-all duration-300">
          <div>
            <span className="block text-[10px] font-bold text-zinc-500 tracking-wider">TOTAL ACTIVE BOARDS</span>
            <span className="block text-3xl font-black text-foreground mt-1.5">{stats.projects}</span>
            <span className="text-[9px] text-zinc-400 font-medium block mt-1">Operational sprint boards</span>
          </div>
        ...
        <div className="glass-panel p-5 rounded-2xl relative overflow-hidden flex items-center justify-between group hover:border-secondary/30 transition-all duration-300">
          <div>
            <span className="block text-[10px] font-bold text-zinc-500 tracking-wider">COMPLETED TASKS</span>
            <span className="block text-3xl font-black text-foreground mt-1.5">{stats.completedTasks}</span>
            <span className="text-[9px] text-zinc-400 font-medium block mt-1">out of {stats.tasks} total items</span>
          </div>
        ...
        <div className="glass-panel p-5 rounded-2xl relative overflow-hidden flex items-center justify-between group hover:border-primary/30 transition-all duration-300">
          <div>
            <span className="block text-[10px] font-bold text-zinc-500 tracking-wider">PENDING TASKS</span>
            <span className="block text-3xl font-black text-foreground mt-1.5">{stats.tasks - stats.completedTasks < 0 ? 0 : stats.tasks - stats.completedTasks}</span>
            <span className="text-[9px] text-zinc-400 font-medium block mt-1">Backlog tasks queue</span>
          </div>
          <div className="h-12 w-12 rounded-xl bg-primary/10 text-primary flex items-center justify-center group-hover:scale-110 transition duration-300">
        ...
        <div className="glass-panel p-5 rounded-2xl relative overflow-hidden flex items-center justify-between group hover:border-red-500/30 transition-all duration-300">
          <div>
            <span className="block text-[10px] font-bold text-red-500 dark:text-red-400 tracking-wider">CRITICAL OVERDUE</span>
            <span className="block text-3xl font-black text-red-600 dark:text-red-400 mt-1.5">
              {checklistTasks.filter(t => t.status === 'OVERDUE' && !t.done).length}
            </span>
            <span className="text-[9px] text-zinc-400 font-medium block mt-1">Requires immediate review</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground flex items-center gap-1.5">
                AI Risk Shield Monitor <Sparkles className="h-3.5 w-3.5 text-primary animate-pulse" />
              </h3>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 font-light">Gemini real-time bottleneck & delay risk scanning engine.</p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold text-zinc-500">OVERALL THREAT INDEX:</span>
            <span className={`px-3 py-1 rounded-full text-[10px] font-bold tracking-wider ${aiRisk?.overallRisk === 'HIGH' ? 'bg-red-500/20 text-red-500 dark:text-red-400 border border-red-500/30 animate-pulse' : aiRisk?.overallRisk === 'MEDIUM' ? 'bg-yellow-500/20 text-yellow-600 dark:text-yellow-400 border border-yellow-500/30' : 'bg-green-500/20 text-green-500 border border-green-500/30'}`}>
              {loadingRisk ? 'SCANNING WORKSPACE...' : aiRisk?.overallRisk || 'LOW'}
            </span>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-3">
            {aiRisk?.riskFactors && aiRisk.riskFactors.length > 0 ? (
              aiRisk.riskFactors.map((rf, idx) => (
                <div key={idx} className="flex gap-3 text-xs bg-card/25 p-3 rounded-lg border border-card-border/50">
                  <span className="font-bold text-primary shrink-0 uppercase">[{rf.source}]</span>
                  <span className="text-zinc-600 dark:text-zinc-300 font-light">{rf.description}</span>
                  <span className="text-zinc-400 dark:text-zinc-500 shrink-0 ml-auto font-semibold">({rf.severity} RISK)</span>
                </div>
              ))
            ) : (
              <div className="text-xs text-zinc-500 italic">No critical bottleneck variables detected. Workspace is running optimally.</div>
            )}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {projects.length > 0 ? (
            filteredProjects.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredProjects.map((p) => (
                  ...
                  >
                    ...
                    <div className="h-2 w-full bg-cover bg-center shrink-0" style={{ backgroundImage: `url(${p.coverImage})` }} />

                    <div className="p-5 flex flex-col justify-between gap-4 flex-1">
                      <div className="flex justify-between items-start gap-2">
                        <Link href={`/workspace/${currentWorkspace?.id}/project/${p.id}`} className="block flex-1 group-hover:text-primary transition">
              ...
              <div className="border border-dashed border-card-border rounded-2xl p-10 text-center bg-card/5 animate-in fade-in duration-200">
                ...
                <p className="text-xs text-zinc-500 font-light mt-1">Try adjusting your filters or search keywords.</p>
              </div>
            )
          ) : (
            <div className="border border-dashed border-card-border rounded-2xl p-12 text-center bg-card/5">
              ...
              <h3 className="text-sm font-bold text-foreground">No active project boards</h3>
              <p className="text-xs text-zinc-500 font-light max-w-sm mx-auto mt-1 mb-6">
                Create your first agile scrum board by clicking the plus (+) icon inside the header project dropdown.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-4">
          <h2 className="text-base font-bold text-foreground flex items-center gap-2">
            <ListTodo className="h-5 w-5 text-secondary" /> Personal Operations Checklist
          </h2>
          ...
          <div className="glass-panel p-5 rounded-2xl border border-card-border bg-card/10 flex flex-col gap-4">
            ...
            {/* Header info */}
            <div>
              <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest block">ACTIVE backlog list</span>
              <span className="text-xs text-zinc-500 mt-0.5 font-light block">Directly check-off tasks to complete developer milestones.</span>
            </div>
              ...
              {checklistTasks.map((t) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="block text-xs font-semibold leading-normal truncate">{t.title}</span>
                    <div className="flex items-center gap-1.5 mt-1.5">
                      <span className={`text-[8px] px-1.5 py-0.2 rounded font-bold uppercase tracking-wider shrink-0 ${t.priority === 'HIGH' ? 'bg-red-500/10 text-red-500' : t.priority === 'MEDIUM' ? 'bg-yellow-500/10 text-yellow-600 dark:text-yellow-400' : 'bg-zinc-500/15 text-zinc-500'}`}>
                        {t.priority}
                      </span>
