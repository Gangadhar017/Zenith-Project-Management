      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2">
            <CalendarIcon className="h-6 w-6 text-primary" /> Workspace Calendar Planner
          </h1>
          <p className="text-xs text-zinc-400 font-light mt-1">Plan and coordinate due dates, scrum sprints, and milestone releases visually.</p>
        </div>
      ...
      <div className="glass-panel rounded-2xl border-white/5 overflow-hidden flex flex-col bg-white/[0.01]">
        ...
        <div className="grid grid-cols-7 gap-px bg-white/5">
          {calendarDays.map((day, idx) => {
            if (day === null) {
              return <div key={idx} className="bg-zinc-950/80 aspect-square p-2 border-b border-r border-white/5" />;
            }
            ...
            return (
              ...
              >
                <span className="text-xs font-bold text-zinc-400 group-hover:text-white transition">{day}</span>
