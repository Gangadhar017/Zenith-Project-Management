...
export default function WorkspaceTeamPage() {
  ...
  const fetchMembers = () => {
    ...
    .then(res => res.json())
    .then(data => {
      const activeWs = data.find((w: any) => w.id === workspaceId);
      if (activeWs) {
  ...
  const handleInviteSubmit = async (e: React.FormEvent) => {
    ...
    const token = useAuthStore.getState().token;
    try {
      const res = await fetch(`http://localhost:8000/api/workspaces/${workspaceId}/invite`, {
        method: 'POST',
        headers: {
      ...
      setSuccess(`User successfully added and synchronized with ${role} permissions!`);
      setEmail('');
      fetchMembers();
    } catch (err: any) {
  ...
  const handleRemoveMember = async (userId: string) => {
    ...
    const token = useAuthStore.getState().token;
    try {
      const res = await fetch(`http://localhost:8000/api/workspaces/${workspaceId}/members/${userId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
    ...
    <div className="max-w-7xl mx-auto flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="glass-panel p-5 rounded-2xl relative overflow-hidden flex items-center justify-between group hover:border-primary/30 transition-all duration-300">
          <div>
            <span className="block text-[10px] font-bold text-zinc-500 tracking-wider">TOTAL MEMBERS</span>
            <span className="block text-2xl font-black text-foreground mt-1.5">{members.length} Users</span>
            <span className="text-[9px] text-zinc-400 font-medium block mt-1">Workspace size</span>
          </div>
        ...
        <div className="glass-panel p-5 rounded-2xl relative overflow-hidden flex items-center justify-between group hover:border-secondary/30 transition-all duration-300">
          <div>
            <span className="block text-[10px] font-bold text-zinc-500 tracking-wider">ACTIVE BOARDS</span>
            <span className="block text-2xl font-black text-foreground mt-1.5">{projects.length} Boards</span>
            <span className="text-[9px] text-zinc-400 font-medium block mt-1">Total workspace projects</span>
          </div>
        ...
        <div className="glass-panel p-5 rounded-2xl relative overflow-hidden flex items-center justify-between group hover:border-purple-500/30 transition-all duration-300">
          <div>
            <span className="block text-[10px] font-bold text-zinc-500 tracking-wider">PENDING BACKLOG</span>
            <span className="block text-2xl font-black text-foreground mt-1.5">12 Tasks</span>
            <span className="text-[9px] text-zinc-400 font-medium block mt-1">Assigned milestones backlog</span>
          </div>
        ...
        <div className="lg:col-span-2 glass-panel p-6 md:p-8 rounded-2xl flex flex-col gap-6 relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 border-b border-card-border pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-foreground">Workspace Membership</h3>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 font-light">List of all active developers, admins, and viewers inside this workspace.</p>
              </div>
            </div>
          </div>
          ...
          {filteredMembers.length > 0 ? (
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-card-border bg-card/5">
                  {filteredMembers.map((m) => {
                    ...
                    return (
                      <tr key={m.id} className="hover:bg-card/10 transition-colors">
                        ...
                        <td className="p-4 text-zinc-500 dark:text-zinc-400 font-medium">{emailStr}</td>
                        <td className="p-4">
                          <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold border tracking-wider uppercase ${m.role === 'OWNER' ? 'bg-yellow-500/15 border-yellow-500/30 text-yellow-500' : m.role === 'ADMIN' ? 'bg-purple-500/15 border-purple-500/30 text-purple-400' : 'bg-zinc-500/15 border-zinc-500/30 text-zinc-500'}`}>
                            {m.role}
                          </span>
                        </td>
            ...
            <div className="border border-dashed border-card-border rounded-xl p-8 text-center bg-card/5 animate-in fade-in duration-200">
              ...
              <h3 className="text-xs font-bold text-foreground">No matching workspace members</h3>
              <p className="text-[10px] text-zinc-500 font-light mt-1">Try adjusting your search query keyword.</p>
            </div>
          )}
        ...
        <div className="glass-panel p-6 rounded-2xl flex flex-col gap-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 border-b border-card-border pb-4">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-400 font-light">Add user emails to collaborate.</p>
            </div>
          </div>
          ...
          <form onSubmit={handleInviteSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-bold text-zinc-500">MEMBERSHIP ROLE</label>
              <select 
                className="bg-card border border-card-border rounded-lg text-xs font-semibold p-2.5 text-foreground cursor-pointer"
                value={role}
                onChange={(e: any) => setRole(e.target.value)}
              >
                <option value="MEMBER" className="bg-background">MEMBER (Write & Edit boards)</option>
                <option value="ADMIN" className="bg-background">ADMIN (Full invites & deletions)</option>
                <option value="VIEWER" className="bg-background">VIEWER (Read-only boards)</option>
              </select>
            </div>
