...
export default function ProjectBoardPage() {
  ...
  const [activeTab, setActiveTab] = useState<'kanban' | 'calendar' | 'sprint'>('kanban');
  ...
  const [aiSolverResult, setAiSolverResult] = useState<any>(null);
  const [solvingBug, setSolvingBug] = useState(false);
  const [assigneeRec, setAssigneeRec] = useState<any>(null);
  ...
  const handleDrop = async (e: React.DragEvent, destStatus: TaskStatus) => {
    e.preventDefault();
    ...
    const maxOrder = destTasks.reduce((max, t) => t.order > max ? t.order : max, 0);
  ...
  const handleAddSubtask = async (e: React.FormEvent) => {
    ...
    const token = useAuthStore.getState().token;
    try {
      const res = await fetch(`http://localhost:8000/api/tasks/${selectedTask.id}/subtasks`, {
        method: 'POST',
        headers: {
  ...
  const handleToggleSubtask = async (subtaskId: string, currentStatus: boolean) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setSubtasksList(prev => prev.map(s => s.id === subtaskId ? data : s));
        selectProject(projectId);
      }
    } catch (err) {
      console.error(err);
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    const token = useAuthStore.getState().token;
    try {
      const res = await fetch(`http://localhost:8000/api/tasks/${selectedTask.id}/comments`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleLogTime = async (seconds: number) => {
    ...
    try {
      await fetch(`http://localhost:8000/api/tasks/${selectedTask.id}/timer`, {
        ...
        body: JSON.stringify({ duration: seconds, description: 'Pomodoro Focus Session completed.' })
      });
  ...
  const handleTriggerSprintPlanning = async () => {
    ...
    const token = useAuthStore.getState().token;
    try {
      const backlogItems = tasks.filter(t => t.status === 'BACKLOG' || t.status === 'TODO');
  ...
  return (
    <div className="flex flex-col gap-6 h-full min-w-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-white">{currentProject?.name || 'Loading Project Board...'}</h1>
          <p className="text-xs text-zinc-400 font-light mt-1">{currentProject?.description || 'Build modular platforms.'}</p>
        </div>
      ...
      {activeTab === 'kanban' && (
        <div className="flex-1 grid grid-cols-1 md:grid-cols-5 gap-4 min-h-[500px] overflow-x-auto items-start pb-10">
          ...
          {COLUMNS.map((col) => {
            const columnTasks = tasks.filter(t => t.status === col).sort((a,b) => a.order - b.order);
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between border-b border-white/5 pb-2">
                  <span className={`text-[10px] font-extrabold tracking-wider ${col === 'IN_PROGRESS' ? 'text-primary' : col === 'DONE' ? 'text-green-400' : 'text-zinc-400'}`}>
                    {col.replace('_', ' ')}
                  </span>
                    ...
                    >
                      ...
                      {t.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {t.tags.map((tag, i) => (
                            <span key={i} className="text-[8px] font-bold bg-white/5 text-zinc-400 px-1.5 py-0.5 rounded-md border border-white/5 uppercase">
                              {tag}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="flex justify-between items-center border-t border-white/5 pt-2 mt-1">
                        <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${t.priority === 'URGENT' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : t.priority === 'HIGH' ? 'bg-orange-500/20 text-orange-400' : 'bg-white/5 text-zinc-500'}`}>
                          {t.priority}
                        </span>
                        
                        {t.assignee && (
                          <img 
                            src={t.assignee.image || `https://api.dicebear.com/7.x/adventurer-neutral/svg?seed=${t.assignee.name}`}
                            alt="assignee avatar"
      ...
      {activeTab === 'sprint' && (
        <div className="glass-panel p-6 rounded-2xl border-white/5 max-w-2xl mx-auto flex flex-col gap-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">AI Sprint Planning Optimizer</h3>
              <p className="text-xs text-zinc-400 font-light">Enter details to build a capacity optimized agile sprint via Gemini models.</p>
            </div>
          </div>

          <div className="flex flex-col gap-4 border-t border-white/5 pt-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-zinc-400">SPRINT NAME</label>
              <input 
                type="text" 
            ...
            >
              {sprintLoading ? 'Optimizing velocities...' : 'Optimize Sprint backlog with AI'} <Sparkles className="h-4 w-4 text-white" />
            </button>
          </div>

          {sprintResult && (
            <div className="mt-4 p-4 rounded-xl bg-white/5 border border-white/10 flex flex-col gap-4">
              <span className="text-xs font-bold text-primary block">OPTIMIZATION METRICS GENERATED</span>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/5 p-3 rounded-lg border border-white/5">
                  <span className="block text-[10px] text-zinc-500 font-bold">CAPACITY ASSIGNED</span>
                  <span className="block text-lg font-black text-white mt-1">{sprintResult.recommendedCapacity} points</span>
                </div>
                <div className="bg-white/5 p-3 rounded-lg border border-white/5">
                  <span className="block text-[10px] text-zinc-500 font-bold">RISK COEFFICIENT</span>
                  <span className={`block text-lg font-black mt-1 ${sprintResult.riskScore > 50 ? 'text-red-400' : 'text-green-400'}`}>
              ...
              <div className="flex flex-col gap-2">
                <span className="text-[10px] text-zinc-500 font-bold">SPRINT RECOMMENDATIONS:</span>
                {sprintResult.recommendations?.map((rec: string, i: number) => (
                  <p key={i} className="text-xs text-zinc-300 font-light flex gap-2 items-start">
                    <Sparkles className="h-3.5 w-3.5 text-primary shrink-0 mt-0.5" /> {rec}
                  </p>
                ))}
              </div>
            </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md glass-panel p-6 rounded-2xl border border-white/10 shadow-2xl relative">
            <button className="absolute top-4 right-4 text-zinc-500 hover:text-white" onClick={() => setShowCreateModal(false)}>
              <X className="h-5 w-5" />
            </button>
            <h3 className="text-lg font-bold text-white mb-2 flex items-center gap-2">
              <Plus className="text-primary h-5 w-5" /> Create Task Card
            </h3>
            <p className="text-xs text-zinc-400 mb-6">Add a brand new card inside your column lane.</p>

            <form onSubmit={handleCreateTaskSubmit} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-zinc-400">CARD TITLE</label>
                <input 
                  type="text" 
                  required
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-zinc-400">DESCRIPTION (OPTIONAL)</label>
                <textarea 
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-bold text-zinc-400">TAGS (COMMA SEPARATED)</label>
                  <input 
                    type="text" 
            ...
            <div>
              <div className="flex justify-between items-start gap-4 mb-6">
                <div>
                  ...
                  <h2 className="text-xl font-black text-white mt-2 leading-tight">{selectedTask.title}</h2>
                  <p className="text-xs text-zinc-400 font-light mt-1">{selectedTask.description || 'No detailed specifications.'}</p>
                </div>
                <button 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-white/5 pt-6">
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col gap-3 relative overflow-hidden">
                    <span className="text-[10px] font-bold text-zinc-500 tracking-wider flex items-center gap-1">
                      <Clock className="h-3.5 w-3.5 text-primary" /> POMODORO FOCUS TIMER
                    </span>
                    ...
                    <div className="flex items-center gap-4 py-2">
                      ...
                      <div className="flex gap-1.5">
                        ...
                        >
                          {timerRunning ? <Pause className="h-3 w-3" /> : <Play className="h-3 w-3" />} {timerRunning ? 'Pause' : 'Start'}
                        </button>
                        <button 
                          onClick={() => {
                            setTimerRunning(false);
                  ...
                  <div className="flex flex-col gap-3">
                    <span className="text-[10px] font-bold text-zinc-500 tracking-wider flex items-center gap-1">
                      <CheckSquare className="h-3.5 w-3.5 text-secondary" /> CHECKLIST SUBTASKS
                    </span>

                    <div className="flex flex-col gap-2">
                      {subtasksList.map((st) => (
                        <div key={st.id} className="flex items-center gap-2 bg-white/5 border border-white/5 p-2.5 rounded-lg text-xs">
                          <input 
                            type="checkbox" 
                          ...
                          />
                          <span className={`text-zinc-300 font-light ${st.isCompleted ? 'line-through opacity-55' : ''}`}>
                            {st.title}
                          </span>
                        </div>
                      ))}
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col gap-3">
                    <span className="text-[10px] font-bold text-zinc-500 tracking-wider flex items-center gap-1">
                      <Bot className="h-3.5 w-3.5 text-purple-400" /> AI SMART ASSIGN SUGGESTION
                    </span>
                    ...
                    >
                      {reccingAssignee ? 'Calculating task skills...' : 'Suggest Optimal Assignee'}
                    </button>

                    {assigneeRec && (
                      <div className="p-3 bg-white/5 border border-white/5 rounded-lg flex flex-col gap-1 text-[11px]">
                        <p className="text-zinc-300 font-light">{assigneeRec.reasoning}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col gap-3">
                    <span className="text-[10px] font-bold text-zinc-500 tracking-wider flex items-center gap-1">
                      <Sparkles className="h-3.5 w-3.5 text-primary" /> AI BUG SOLVER DRAFT
                    </span>
                    ...
                    >
                      {solvingBug ? 'Scanning bug trace...' : 'Solve Bug with AI'}
                    </button>
                    ...
                    {aiSolverResult && (
                      <div className="flex flex-col gap-3 text-xs bg-white/5 p-3 rounded-lg border border-white/5 mt-2 max-h-[160px] overflow-y-auto">
                        <div>
                          <span className="text-[9px] font-bold text-primary uppercase block mb-1">ANALYSIS:</span>
                          <p className="text-zinc-300 font-light leading-normal">{aiSolverResult.rootCauseAnalysis}</p>
                        </div>
                        
                        <div>
                          <span className="text-[9px] font-bold text-secondary uppercase block mb-1">CODE FIX DRAFT:</span>
              ...
              <div className="flex flex-col gap-4 border-t border-white/5 pt-6 mt-6">
                <span className="text-[10px] font-bold text-zinc-500 tracking-wider flex items-center gap-1">
                  <MessageSquare className="h-3.5 w-3.5 text-zinc-400" /> COMMENTS STREAM
                </span>
                ...
                <div className="flex flex-col gap-3 max-h-[160px] overflow-y-auto p-1">
                  {commentsList.map((comm) => (
                    <div key={comm.id} className="flex gap-3 text-xs">
                      <img 
                        src={comm.user?.image || `https://api.dicebear.com/7.x/adventurer-neutral/svg?seed=${comm.user?.name}`} 
                        alt="avatar" 
                      ...
                      <div className="flex-1 bg-white/5 border border-white/5 p-3 rounded-xl">
                        <div className="flex justify-between items-center mb-1">
                          <span className="font-bold text-white text-[11px]">{comm.user?.name}</span>
                          <span className="text-[8px] text-zinc-500">{new Date(comm.createdAt).toLocaleTimeString()}</span>
                        </div>
                        <p className="text-zinc-300 font-light leading-relaxed">{comm.content}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {isTyping[selectedTask.id] && isTyping[selectedTask.id].isTyping && (
                ...
                <form onSubmit={handleAddComment} className="flex gap-2">
                  <input 
                    ...
                    onChange={(e) => {
                      ...
                      emitTyping(projectId, selectedTask.id, user!.name, e.target.value.length > 0);
                    }}
                    onBlur={() => emitTyping(projectId, selectedTask.id, user!.name, false)}
                  />
                  <button 
                    type="submit"
