...
export default function WorkspaceWikiPage() {
  ...
  const { documents, fetchDocuments, createDocument, updateDocument } = useWorkspaceStore();
  ...
  const handleCreateDocSubmit = async (e: React.FormEvent) => {
    ...
    await createDocument(newTitle, '# ' + newTitle + '\n\nStart writing documentation here...', newIsWiki, workspaceId);
    setNewTitle('');
  ...
  const handleTriggerMeetingNotes = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/ai/summarize-meeting', {
        method: 'POST',
        headers: {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 max-w-6xl mx-auto h-full items-start">
      ...
      <div className="glass-panel p-5 rounded-2xl border-white/5 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5 max-h-[50vh] overflow-y-auto">
          ...
          {documents.length === 0 && (
            <span className="text-[10px] text-zinc-500 italic p-3 text-center">No docs generated yet.</span>
          )}
        </div>
      </div>
      ...
      <div className="lg:col-span-3 flex flex-col gap-6">
        ...
        {activeDoc ? (
          ...
          <div className="glass-panel p-12 rounded-2xl border-white/5 border-dashed text-center bg-white/[0.01] flex flex-col items-center justify-center min-h-[40vh]">
            ...
            <h3 className="text-sm font-bold text-zinc-400">Select a Knowledge Document</h3>
            <p className="text-xs text-zinc-500 font-light max-w-sm mt-1">
              Read collaborative documents or click the plus (+) button to draft active company resources.
            </p>
          </div>
        )}
        ...
        <div className="glass-panel p-6 rounded-2xl border-white/5 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="text-base font-bold text-white flex items-center gap-1">
                AI Meeting Transcript Summarizer <Sparkles className="h-3.5 w-3.5 text-primary" />
              </h3>
              <p className="text-xs text-zinc-400 font-light">Convert video standup notes directly into standard sprint backlog cards.</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            >
              {sumLoading ? 'Scanning vocal metrics...' : 'Summarize and extract action items'} <ArrowRight className="h-3.5 w-3.5" />
            </button>
          </div>

          {summaryResult && (
            <div className="mt-4 p-4 rounded-xl bg-white/5 border border-white/10 flex flex-col gap-4">
              <span className="text-xs font-bold text-secondary block">EXTRACTED MEETING SUMMARY</span>
              <p className="text-xs text-zinc-300 font-light leading-normal">{summaryResult.summary}</p>
              
              <div className="flex flex-col gap-2">
                <span className="text-[10px] text-zinc-500 font-bold">ACTION ITEMS DETECTED:</span>
                {summaryResult.actionItems?.map((act: any, i: number) => (
                  <div key={i} className="flex justify-between items-center bg-white/[0.01] p-3 rounded-lg border border-white/5 text-xs">
                    <div className="flex gap-2">
                      <span className="font-bold text-primary">[{act.priority}]</span>
                      <span className="text-zinc-300 font-light">{act.taskTitle}</span>
                    </div>
                    <span className="text-zinc-500 font-semibold italic">Suggest: {act.assigneeSuggestion}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {showNewDocModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md glass-panel p-6 rounded-2xl border border-white/10 shadow-2xl relative">
            <button className="absolute top-4 right-4 text-zinc-500 hover:text-white" onClick={() => setShowNewDocModal(false)}>
              <X className="h-5 w-5" />
            </button>
            <h3 className="text-lg font-bold text-white mb-2 flex items-center gap-2">
              <Plus className="text-primary h-5 w-5" /> New Wiki Document
            </h3>
            <p className="text-xs text-zinc-400 mb-6">Create resources, roadmap lists, or team guides.</p>

            <form onSubmit={handleCreateDocSubmit} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-zinc-400">DOCUMENT TITLE</label>
                <input 
                  type="text" 
                  required
