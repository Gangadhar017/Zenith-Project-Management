...
export default function WorkspaceAnalyticsPage() {
  ...
  const handleTriggerInsights = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/ai/productivity-insights', {
    ...
    <div className="flex flex-col gap-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2">
            <LineChart className="h-6 w-6 text-primary" /> Velocity Analytics Hub
          </h1>
          <p className="text-xs text-zinc-400 font-light mt-1">Aggregated statistics, burn-down metrics, and AI health scores.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-6 rounded-2xl border-white/5 flex flex-col gap-4">
          <span className="text-xs font-bold text-white tracking-wide block">SPRINT BURN-DOWN SPEED</span>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <RechartsLineChart data={burndownData}>
                ...
                />
                <Line type="monotone" dataKey="remaining" stroke="#a855f7" strokeWidth={3} dot={{ fill: '#3b82f6', r: 4 }} />
              </RechartsLineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="glass-panel p-6 rounded-2xl border-white/5 flex flex-col gap-4">
          <span className="text-xs font-bold text-white tracking-wide block">KANBAN LANES ALLOCATIONS</span>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="glass-panel p-6 rounded-2xl border-white/5 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-4 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white flex items-center gap-1.5">
                AI Productivity Insights <Sparkles className="h-3.5 w-3.5 text-primary" />
              </h3>
              <p className="text-xs text-zinc-400 font-light">Agile workspace logs analyzed dynamically by Gemini engine models.</p>
            </div>
          </div>
          ...
          >
            {loadingInsights ? 'Analyzing focus patterns...' : 'Regenerate Analysis'}
          </button>
        </div>
          ...
          insights && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs">
              ...
              <div className="bg-white/5 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                <div>
                  <span className="block text-[10px] text-zinc-500 font-bold mb-1 uppercase">EFFICIENCY RATING</span>
                  <p className="text-2xl font-black text-white">{insights.efficiencyScore}%</p>
                  <p className="text-[10px] text-zinc-400 font-light mt-2 leading-relaxed">
                    Overall performance score determined based on sprint completion speeds and resolution volumes.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white/5 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                <div>
                  <span className="block text-[10px] text-zinc-500 font-bold mb-1 uppercase">FOCUS METRIC</span>
                  <p className="text-2xl font-black text-white">{insights.focusHoursPerDay} hrs/day</p>
                  <p className="text-[10px] text-zinc-400 font-light mt-2 leading-relaxed">
                    Average continuous deep work focus duration logged through task Pomodoro widgets.
                  </p>
                </div>
              </div>

              <div className="bg-white/5 p-4 rounded-xl border border-white/5 flex flex-col gap-3">
                <span className="block text-[10px] text-zinc-500 font-bold uppercase">ACTIONABLE ADVICE:</span>
                {insights.actionableTips?.map((tip: string, i: number) => (
                  <p key={i} className="text-zinc-300 font-light leading-relaxed flex gap-2">
                    <Sparkles className="h-3.5 w-3.5 text-primary shrink-0 mt-0.5" /> {tip}
                  </p>
                ))}
              </div>
