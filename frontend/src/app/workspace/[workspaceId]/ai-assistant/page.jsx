  ...
  const [chatLogs, setChatLogs] = useState([
    {
      sender: "ZENITH",
      content:
        "Hello! I am Zenith Brain, your enterprise agile assistant. Ask me anything about active projects, card assignments, or document wikis.",
    },
  ]);
  ...
  const handleChatSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setChatLogs((prev) => [
          ...prev,
          { sender: "ZENITH", content: data.reply },
        ]);
      }
    } catch (err) {
      console.error(err);
      ...
      <div className="lg:col-span-2 glass-panel rounded-2xl border-white/5 flex flex-col h-[75vh] relative overflow-hidden">
        ...
        <div className="p-4 border-b border-white/5 flex items-center gap-3">
          ...
          <div>
            <span className="block text-sm font-bold text-white">
              Zenith Workspace AI Brain
            </span>
            <span className="block text-[10px] text-zinc-500 font-light">
              Interactive chatbot synchronized across all cards.
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
          {chatLogs.map((chat, idx) => (
            ...
            >
              <div
                className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 border border-white/5 ${chat.sender === "USER" ? "bg-zinc-800" : "bg-primary/20 text-primary"}`}
              >
                {chat.sender === "USER" ? "U" : <Bot className="h-4.5 w-4.5" />}
              </div>
              <div
                className={`p-3.5 rounded-2xl leading-relaxed font-light ${chat.sender === "USER" ? "bg-primary text-white rounded-tr-none" : "bg-white/5 text-zinc-300 border border-white/5 rounded-tl-none"}`}
              >
                {chat.content}
              </div>
            </div>
        ...
        <form
          ...
          className="p-4 border-t border-white/5 bg-white/[0.01] flex gap-2"
        >
          <input
            type="text"
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="glass-panel p-5 rounded-2xl border-white/5 relative overflow-hidden">
          ...
          <h3 className="text-sm font-bold text-white mb-1 flex items-center gap-1.5">
            <Sparkles className="h-4 w-4 text-secondary animate-pulse" /> AI
            Standup Generator
          </h3>
          <p className="text-[10px] text-zinc-400 font-light mb-4">
            Compose your daily standup text based on actual database logs.
          </p>
          ...
          {standup && (
            <div className="mt-4 p-3 bg-white/5 border border-white/5 rounded-lg flex flex-col gap-3 text-xs">
              <div>
                <span className="block text-[9px] font-bold text-secondary uppercase mb-0.5">
                  YESTERDAY:
                </span>
        ...
        <div className="glass-panel p-5 rounded-2xl border-white/5 relative overflow-hidden">
          ...
          <p className="text-[10px] text-zinc-400 font-light mb-4">
            Generate executive metrics reports for select scrum boards.
          </p>
              ...
              <div className="p-3 bg-white/5 border border-white/5 rounded-lg flex flex-col gap-3 text-xs mt-1">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-extrabold text-green-400">
                    {projSummary.healthScore}% HEALTH
                  </span>
                </div>
                <div>
                  <span className="block text-[9px] font-bold text-primary uppercase mb-0.5">
                    EXECUTIVE NOTE:
                  </span>
                  <p className="text-zinc-300 font-light leading-normal">
                    {projSummary.executiveSummary}
                  </p>
                </div>
                <div className="flex flex-col gap-1.5">
                  ...
                  {projSummary.milestones?.map((ms, i) => (
                    <p
                      key={i}
                      className="text-[10px] text-zinc-300 font-light flex gap-1.5"
                    >
                      • {ms}
                    </p>
                  ))}
